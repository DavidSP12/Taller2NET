  ...
  try {
    ...
    document.getElementById('studentsTable').innerHTML = items.length ? `<table>
      ...
      <tbody>${items.map(s => `<tr>
        ...
        <td><span style="color:var(--${(s.averageGrade ?? 0) >= 3 ? 'green' : 'red'})">${s.averageGrade?.toFixed(2) ?? 'N/A'}</span></td>
  ...
  try {
    ...
    document.getElementById('topCoursesCards').innerHTML = items.map((c, i) => {
      ...
      return `<div class="info-card">
        ...
        <div class="detail"><span class="detail-label">Inscritos</span><span class="detail-value">${c.enrolledStudents ?? c.enrollmentCount ?? 0}</span></div>
        <div class="detail"><span class="detail-label">Máx. Estudiantes</span><span class="detail-value">${c.maxStudents ?? '-'}</span></div>
        <div class="progress-bar"><div class="progress-fill progress-accent" style="width:${c.maxStudents ? Math.min(((c.enrolledStudents ?? c.enrollmentCount ?? 0) / c.maxStudents) * 100, 100) : 50}%"></div></div>
      </div>`;
...
async function loadAttendance() {
  ...
  try {
    ...
    const total = (d.presentCount ?? 0) + (d.absentCount ?? 0) + (d.lateCount ?? 0) + (d.excusedCount ?? 0);
    ...
    document.getElementById('attendanceContent').innerHTML = `
      <div class="stats-grid">
        <div class="stat-card"><div class="stat-label">Total Registros</div><div class="stat-value blue">${d.totalRecords ?? total}</div></div>
        <div class="stat-card"><div class="stat-label">Tasa General</div><div class="stat-value green">${d.overallRate?.toFixed(1) ?? pctPresent}%</div></div>
        <div class="stat-card"><div class="stat-label">✅ Presentes</div><div class="stat-value green">${d.presentCount ?? 0} (${pctPresent}%)</div>
          <div class="progress-bar"><div class="progress-fill progress-green" style="width:${pctPresent}%"></div></div></div>
        <div class="stat-card"><div class="stat-label">❌ Ausentes</div><div class="stat-value" style="color:var(--red)">${d.absentCount ?? 0} (${pctAbsent}%)</div>
          <div class="progress-bar"><div class="progress-fill" style="width:${pctAbsent}%;background:var(--red)"></div></div></div>
        <div class="stat-card"><div class="stat-label">⏰ Tardanzas</div><div class="stat-value yellow">${d.lateCount ?? 0} (${pctLate}%)</div>
          <div class="progress-bar"><div class="progress-fill" style="width:${pctLate}%;background:var(--yellow)"></div></div></div>
        <div class="stat-card"><div class="stat-label">📝 Excusas</div><div class="stat-value cyan">${d.excusedCount ?? 0} (${pctExcused}%)</div>
          <div class="progress-bar"><div class="progress-fill" style="width:${pctExcused}%;background:var(--cyan)"></div></div></div>
      </div>`;
  } catch (e) { console.error(e); }
  showLoading(false);
}
...
async function loadGrades() {
  ...
  try {
    ...
    document.getElementById('gradesContent').innerHTML = `
      <div class="stats-grid">
        <div class="stat-card"><div class="stat-label">Promedio General</div><div class="stat-value accent">${d.overallAverage?.toFixed(2) ?? 'N/A'}</div></div>
        <div class="stat-card"><div class="stat-label">Nota Más Alta</div><div class="stat-value green">${d.highestGrade?.toFixed(2) ?? 'N/A'}</div></div>
        <div class="stat-card"><div class="stat-label">Nota Más Baja</div><div class="stat-value" style="color:var(--red)">${d.lowestGrade?.toFixed(2) ?? 'N/A'}</div></div>
        <div class="stat-card"><div class="stat-label">Total Calificaciones</div><div class="stat-value blue">${d.totalGrades ?? 0}</div></div>
        <div class="stat-card"><div class="stat-label">Aprobados (≥3.0)</div><div class="stat-value green">${d.passingCount ?? 'N/A'}</div></div>
        <div class="stat-card"><div class="stat-label">Reprobados (<3.0)</div><div class="stat-value" style="color:var(--red)">${d.failingCount ?? 'N/A'}</div></div>
      </div>`;
  } catch (e) { console.error(e); }
  showLoading(false);
}
    ...
    document.getElementById('activityTimeline').innerHTML = items.length ? items.map(a => {
      ...
      return `<div class="timeline-item">
        ...
        <div class="timeline-body">
          ...
          <div class="timeline-detail">${a.details || a.studentName || ''} ${a.courseName ? '· ' + a.courseName : ''}</div>
        </div>
  ...
  try {
    ...
    document.getElementById('programsCards').innerHTML = items.length ? items.map(p => `
      <div class="info-card">
        ...
        <div class="progress-bar"><div class="progress-fill progress-green" style="width:${p.attendanceRate ?? 0}%"></div></div>
      </div>
    `).join('') : '<p style="color:var(--text-muted);padding:20px">No hay datos</p>';
  } catch (e) { console.error(e); }
  showLoading(false);
